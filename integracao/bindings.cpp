...
std::vector<std::vector<float>> calcular_similaridade(Historico &historico,
                                                      int fast) {
...
recomendar_produtos(Historico &historico,
                    std::vector<std::vector<float>> &matrixSim, int idCliente,
                    int k) {
...
PYBIND11_MODULE(pyrecommend, m) {
  ...
  m.def("calcular_similaridade", &calcular_similaridade, py::arg("historico"),
        py::arg("fast") = 0);

  m.def("recomendar_produtos", &recomendar_produtos, py::arg("historico"),
        py::arg("matrixSim"), py::arg("idCliente"), py::arg("k"));

  m.def("benchmark", &benchmark, py::arg("historico"));
}
